...
export default function ApprovalsPage() {
  ...
  // V12 阶段 3.2: memoize columns（引用均为稳定函数/常量）
  const columns = useMemo(() => [
    ...
    { title: "状态", dataIndex: "status", key: "status", width: 80, render: (v: string) => <Badge status={(statusColor[v] || "default") as import("antd").BadgeProps["status"]} text={v} /> },
    { title: "进度", key: "progress", width: 80, render: (_: any, r: Approval) => `${r.approvals?.length || 0} / ${r.required}` },
    { title: "创建时间", dataIndex: "created_at", key: "created_at", width: 160, render: (v: string) => v ? formatBeijing(v) : "-" },
