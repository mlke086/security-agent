...
_SAMPLE_TEXTS = [
    (
        ...
        {"ip": ["203.0.113.5"], "command": ["whoami", "id"]},
        "honeypot",
        "true_positive",
        0.92,
        "high",
    ...
    for text, iocs, source, verdict, confidence, priority, evidence in _SAMPLE_TEXTS:
        ...
        await store.add_trace_step(
            ...
            TraceStep(
                node="entry",
                action="received",
                summary=f"Event received from {source}",
                timestamp=t,
        ...
        await store.add_trace_step(
            ...
            TraceStep(
                node="orchestrator",
                action="triage",
                summary=f"priority={priority} tags={evidence}",
                timestamp=t,
                details={
                    "priority": priority,
                    "tags": ["security_event"],
                    "reasoning": evidence[0] if evidence else "",
                },
            ),
        )
        await store.add_trace_step(
        ...
        await store.add_trace_step(
            ...
            TraceStep(
                node="aggregator",
                action="aggregate",
                summary=f"Aggregated with verdict={verdict}",
                timestamp=t,
        ...
        if verdict == "true_positive" and confidence >= 0.8:
            ...
            await store.add_trace_step(
                ...
                TraceStep(
                    ...
                    summary="Matched playbook for high-confidence event",
                    timestamp=t,
