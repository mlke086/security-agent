...
EventStatus = Literal[
    "processing", "completed", "pending_approval", "ignored", "error", "rejected"
]
...
class MemoryEventStore:
    ...
    async def create_event(self, event_id: str, text: str, iocs: dict, source: str) -> EventRecord:
        rec = EventRecord(event_id=event_id, source=source,
    ...
    async def list_events(
        self, status: str | None = None, verdict: str | None = None,
        priority: str | None = None, limit: int = 50, offset: int = 0,
    ) -> list[EventRecord]:
        items = [e for e in self._events.values()
    ...
    async def metrics(self) -> dict[str, Any]:
        ...
        return {"total_events": len(events), "by_verdict": dict(verdicts),
                "by_priority": dict(priorities), "pending_approvals": pending,
    ...
    async def add_approval(self, event_id: str, approval: ApprovalEntry) -> None:
        async with await self._pg_conn() as conn:
            await conn.execute(
                ...
                "updated_at = NOW() WHERE event_id=$2",
                approval.model_dump_json(), event_id)
    ...
    async def metrics(self) -> dict:
        async with await self._pg_conn() as conn:
            ...
            pending = await conn.fetchval(
                "SELECT COUNT(*) FROM events WHERE data->>'status' = 'pending_approval'") or 0
            avg_duration = await conn.fetchval(
                "SELECT COALESCE(AVG((data->>'duration_ms')::int), 0) FROM events") or 0
        return {
            "total_events": total,
