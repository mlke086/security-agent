...
class DnsBlockConnector:
    ...
    async def execute(self, op: dict, ctx: ActionContext) -> ActionResult:
        ...
        if not domain:
            return ActionResult(
                op_id=op_id, op_type="dns_block", status="skipped", output="No domain specified"
            )
        ...
        try:
            ...
            try:
                ...
                await r.aclose()  # V12 5.6: exception-safe close (was leaking on failure)
            return ActionResult(
                op_id=op_id,
                op_type="dns_block",
                status="success",
        ...
        if domain:
            try:
                ...
                try:
                    ...
                    await r.aclose()  # V12 5.6: exception-safe close
            except Exception as exc:
