...
阶段 1 迁入:从 src/orchestration/memory/manager.py 原样迁移。
MemoryManager 直连 Milvus + Neo4j,是 graphrag 服务的内部方法,
不再由 scan-engine 直接 import(改通过 HTTP /memory/add /memory/search 访问)。
