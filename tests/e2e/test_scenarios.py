"""S3-5: End-to-end scenario tests covering all 5 routing paths.

Run as a standalone script (``python tests/e2e/test_scenarios.py``). It is NOT
collected by pytest on purpose - the scenarios drive the real LangGraph pipeline
(LLM + subgraphs) and would be too slow/flaky for the unit suite. The contract
tests live in tests/integration/test_investigation.py.
...
# Settings validation requires an API secret key (>=16 chars) at import time.
os.environ.setdefault("API_SECRET_KEY", "e2e-test-secret-key-000000")
...
def submit_event(payload: dict, token: str) -> dict:
    """Submit synchronously (?sync=true) so the pipeline completes before returning."""
    return client.post("/api/v1/events?sync=true", json=payload, headers=headers(token))
...
def main() -> None:
    ...
    eid1 = data["event_id"]
    resp = client.get(f"/api/v1/events/{eid1}", headers=headers(admin_token))
    check("1", "Event detail endpoint", resp.status_code == 200)
    detail = resp.json()
    check("1", "Event reached terminal status", detail.get("status") in TERMINAL_STATUSES)
    check("1", "Honeypot triaged as high priority", detail.get("priority") == "high")
    ...
    eid2 = data["event_id"]
    resp = client.get(f"/api/v1/events/{eid2}", headers=headers(admin_token))
    check("2", "CVE event detail endpoint", resp.status_code == 200)
    check("2", "CVE event reached terminal status", resp.json().get("status") in TERMINAL_STATUSES)
    ...
    check("4", "Metrics counts submitted events", metrics.get("total_events", 0) >= 3)
    check("4", "Metrics has by_verdict breakdown", "by_verdict" in metrics)
    ...
    resp = client.get("/api/v1/auth/me", headers=headers(admin_token))
