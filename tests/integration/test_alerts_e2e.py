...
def test_01_wazuh_ingest(client, admin_headers):
    r = client.post(
        "/api/v1/alerts/ingest", json={"source": "wazuh", "payload": WAZUH}, headers=admin_headers
    )
...
def test_02_elkeid_ingest(client, admin_headers):
    r = client.post(
        "/api/v1/alerts/ingest", json={"source": "elkeid", "payload": ELKEID}, headers=admin_headers
    )
    assert r.status_code == 200, f"ingest failed: {r.text}"
...
def test_03_syslog_ingest(client, admin_headers):
    r = client.post(
        "/api/v1/alerts/ingest", json={"source": "syslog", "payload": SYSLOG}, headers=admin_headers
    )
    assert r.status_code == 200, f"ingest failed: {r.text}"
...
def test_05_list_with_severity_filter(client, admin_headers):
    r = client.get(
        "/api/v1/alerts", params={"severity": "critical", "limit": 50}, headers=admin_headers
    )
...
def test_06_list_with_source_filter(client, admin_headers):
    r = client.get("/api/v1/alerts", params={"source": "wazuh", "limit": 50}, headers=admin_headers)
    assert r.status_code == 200
    items = r.json()["items"]
    for a in items:
...
def test_08_patch_status_persists(client, admin_headers):
    ...
    r = client.patch(
        f"/api/v1/alerts/{aid}/status", json={"status": "acknowledged"}, headers=admin_headers
    )
    assert r.status_code == 200
...
def test_09_rbac_viewer_cannot_patch(client, viewer_headers):
    aid = "wazuh:e2e-wazuh-001"
    r = client.patch(
        f"/api/v1/alerts/{aid}/status", json={"status": "resolved"}, headers=viewer_headers
    )
...
def test_11_patch_invalid_status_rejected(client, admin_headers):
    aid = "wazuh:e2e-wazuh-001"
    r = client.patch(
        f"/api/v1/alerts/{aid}/status", json={"status": "bogus"}, headers=admin_headers
    )
