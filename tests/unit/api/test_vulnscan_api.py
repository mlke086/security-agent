...
class TestResultsExtendedFilters:
    ...
    def test_extended_filters_forwarded_to_store(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs:
            ...
            kwargs = store.list_vulns.await_args.kwargs
            assert kwargs["cve"] == "CVE-2024-0001"
            assert kwargs["cve_keyword"] == "2024"
            assert kwargs["hostname_keyword"] == "web"
            assert kwargs["name_keyword"] == "xss"
            assert kwargs["ai_processed"] is True
            assert kwargs["date_from"] == "2026-07-01T00:00:00Z"
            assert kwargs["date_to"] == "2026-07-29T23:59:59Z"
    ...
    def test_group_filter_pushes_hostnames_server_side(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs:
            ...
            store.list_hosts = AsyncMock(return_value=[
                Host(agent_id="a1", hostname="order-1", ip="10.0.0.1",
                     os="linux", arch="amd64", kernel="5.x", group="order-svc"),
            ])
            # Server-side filter returns only the matching vuln.
            v_in = VulnFinding(
                finding_id="f-in", task_id="t", agent_id="a", hostname="order-1",
                category=ScanModule.SYS_VULN, name="x", severity="high",
        ...
        kwargs = store.list_vulns.await_args.kwargs
        assert kwargs["hostnames"] == ["order-1"]
    ...
    def test_patch_to_fixed_records_fix_time(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs:
            ...
            existing = VulnFinding(
                finding_id="f-1", task_id="t", agent_id="a", hostname="web-1",
                category=ScanModule.SYS_VULN, name="x", severity="high",
                detected_at="2026-07-01T00:00:00",
            )
    ...
    def test_host_stats_returns_per_group_breakdown(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs, \
            ...
            store.list_groups = AsyncMock(return_value=[
                {"name": "order-svc", "member_count": 2, "description": "", "created_at": "", "origin": "managed"},
                {"name": "billing-svc", "member_count": 1, "description": "", "created_at": "", "origin": "managed"},
            ])
            async def _list_hosts(group=None, **_):
                ...
                all_hosts = [
                    Host(agent_id="a1", hostname="order-1", ip="10.0.0.1", os="linux", arch="amd64", kernel="5.x", group="order-svc"),
                    Host(agent_id="a2", hostname="order-2", ip="10.0.0.2", os="linux", arch="amd64", kernel="5.x", group="order-svc"),
                    Host(agent_id="a3", hostname="billing-1", ip="10.0.0.3", os="linux", arch="amd64", kernel="5.x", group="billing-svc"),
                ]
                if group == "order-svc":
            ...
            v1 = VulnFinding(finding_id="f1", task_id="t", agent_id="a1", hostname="order-1", category=ScanModule.SYS_VULN, name="x", severity="high", detected_at="2026-07-01T00:00:00")
            v2 = VulnFinding(finding_id="f2", task_id="t", agent_id="a2", hostname="order-2", category=ScanModule.SYS_VULN, name="y", severity="critical", detected_at="2026-07-01T00:00:00")
            v3 = VulnFinding(finding_id="f3", task_id="t", agent_id="a3", hostname="billing-1", category=ScanModule.SYS_VULN, name="z", severity="low", detected_at="2026-07-01T00:00:00")
            v4 = VulnFinding(finding_id="f4", task_id="t", agent_id="a9", hostname="misc-1", category=ScanModule.SYS_VULN, name="q", severity="medium", detected_at="2026-07-01T00:00:00")
...
class TestVulnDetailHostMeta:
    ...
    def test_get_vuln_includes_host_meta_by_agent_id(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs:
            store = _mock_store()
            v = VulnFinding(
                finding_id="f-1", task_id="t", agent_id="agent-a", hostname="order-1",
                category=ScanModule.SYS_VULN, name="x", severity="high",
                detected_at="2026-07-01T00:00:00",
            )
            host = Host(
                agent_id="agent-a", hostname="order-1", ip="10.0.0.1", os="linux",
                arch="amd64", kernel="5.x", group="order-svc", owner="alice",
                env="prod",
            )
    ...
    def test_get_vuln_falls_back_to_hostname(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs:
            ...
            host = Host(
                agent_id="agent-b", hostname="order-1", ip="10.0.0.1", os="linux",
                arch="amd64", kernel="5.x", group="order-svc",
            )
            store.get_vuln = AsyncMock(return_value=v)
    ...
    def test_host_stats_calls_store_a_fixed_number_of_times(self):
        ...
        with patch("src.api.routers.vulnscan.get_vulnscan_store") as mock_vs:
            store = _mock_store()
            store.list_groups = AsyncMock(return_value=[
                {"name": f"g-{i}", "member_count": 0, "description": "", "created_at": "", "origin": "managed"}
                for i in range(20)
            ])
            store.list_hosts = AsyncMock(return_value=[])
            store.list_vulns = AsyncMock(return_value=[])
