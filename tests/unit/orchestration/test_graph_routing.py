...
class TestRouteAfterVerdict:
    ...
    def _state(self, **over):
        base = dict(
            event_id="1", stage="route", subgraph_result={}, pending_action=None, error=None
        )
        base.update(over)
        return base
...
class TestVulnCheckVerdict:
    ...
    async def test_vulnerable_event_gets_true_positive_verdict(self):
        ...
        mock_subgraph.ainvoke = AsyncMock(
            return_value={
                "is_vulnerable": True,
                "final_poc": "import requests",
                "exploit_chain": "x",
            }
        )
        with patch(
            "src.orchestration.subgraphs.vuln_hunter.graph.build_vuln_hunter_subgraph",
            return_value=mock_subgraph,
        ):
            result = await graph_mod.vuln_check_node(
                {
                    "event_id": "e1",
                    "raw_event": {"sanitized_text": "x"},
                }
            )
